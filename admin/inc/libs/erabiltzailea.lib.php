...
class erabiltzailea {
	...
	public function get_erabiltzailea (){
		return (get_dbtable_field_by_id ("administrazioa", "erabiltzailea", $this->erabiltzailea_id));
	}
	...
	public function logout (){
		...
		$sql = "DELETE FROM sesioak WHERE fk_erabiltzailea='" . $this->erabiltzailea_id . "' AND session_id='" . session_id () . "'";
		$this->dbo->query ($sql) or die ($this->dbo->ShowError ());
	...
	public function logged (){
		if (is_dbtable_id ("administrazioa", $this->erabiltzailea_id)){
			// Comprobamos si se ha iniciado sesion...
			$sql = "SELECT * FROM sesioak WHERE fk_erabiltzailea='" . $this->erabiltzailea_id . "' AND session_id='" . session_id () . "'";
			$this->dbo->query ($sql) or die ($this->dbo->ShowError ());
			if ($this->dbo->emaitza_kopurua () == 1){
				...
				if ((time () - $rowSesioa["time"]) < 1800){
					...
					$sql = "UPDATE sesioak SET time = '" . time () . "' WHERE fk_erabiltzailea='" . $this->erabiltzailea_id . "' AND session_id='" . session_id () . "'";
					$this->dbo->query ($sql) or die ($this->dbo->ShowError ());
